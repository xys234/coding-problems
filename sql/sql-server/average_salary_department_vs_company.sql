...
SELECT dept_avg.pay_month AS pay_month, 
department_id, 
(
	CASE
...
FROM
(
	SELECT LEFT(pay_date, 7) AS pay_month, department_id, AVG(amount) AS d_avg
	FROM salary s JOIN employee e ON
	(
...
(
	SELECT LEFT(pay_date, 7) AS pay_month, AVG(amount) AS c_avg
	FROM salary s JOIN employee e ON
	(
		s.employee_id = e.employee_id
	) 
...
(
	dept_avg.pay_month = cpy_avg.pay_month
)
